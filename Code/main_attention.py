...
#IMPORTANT
classifier = joblib.load('nayaModel.pkl')
import matplotlib.pyplot as plt




X_model = np.loadtxt('nayaX5.txt', dtype=float)

y = np.loadtxt('nayaY3.txt', dtype=int)                 
...
while (datetime.datetime.now().minute-first_min)<20:
    blinkStrength=0
    ...
    if len(line) > 20:
        ...
        if "blinkStrength" in dict:
            ...
            if blinkStrength>100 and blinkStrength_values[-1]==0 and blinkStrength_values[-2]<=80:
                db.child("Screen Shoot").set(0)
                db.child("Saved").set(0)
                
                db.child("Screen Shoot").set(1)
                myScreenshot = pyautogui.screenshot()
                #myScreenshot.save(r'E:\COMPUTER ENGINEERING\4TH YEAR\Project-I & II\Code py\SS\a'+str(iterations)+'_'+str(timediff)+'.png')
                print("Screen Shoot Taken")
                SS=1
            else:
                db.child("Screen Shoot").set(0)
                SS=0
        ...
        if eSenseDict['attention']<50 and eSenseDict['attention']>0:
            dcnt+=1
            if dcnt>=3:
                pause=1
                icnt=0
            ...
            if icnt>=3:
                ...
                dcnt=0
        if last_pause != pause:
            data={"pause":pause,"Attention":eSenseDict['attention'],"Avg_5":avg_5,"Avg":avg}
            db.child("Concentration").set(data)
        last_pause=pause
        ...
        #values_list.append(np.array([blinkStrength]))
        iterations += 1
        
        list_c.append(eSenseDict['attention'])
        
        tme=datetime.datetime.now()
        if tme.second==0 or tme.second==20 or tme.second==40:
            print("httyt"+" "+str(tme.second))
            avg_5=0
            for i in range(0,len(list_c)):
                avg_5=avg_5+list_c[i]
            avg_5=round(avg_5/len(list_c))
            list_c.clear()
            data={"pause":pause,"Attention":eSenseDict['attention'],"Avg_5":avg_5,"Avg":avg}
            db.child("Concentration").set(data)
        ...
        if iterations ==1 or iterations == 2:
            ...
            if blinkStrength_values[-3] >100 and blinkStrength_values[-2] ==0 and blinkStrength_values[-1]<=80 and SS==1 :
                myScreenshot.save(r'E:\COMPUTER ENGINEERING\4TH YEAR\Project-I & II\Code py\SS\a'+str(iterations)+'_'+str(timediff)+'.png')
                print("Saved")
                db.child("Saved").set(1)
                SS=0
                #db.child("Screen Shoot").set(0)
                #db.child("Saved").set(0) 
        ...
        if iterations == 1 or iterations == 2 or iterations ==3:
            ...
            if blinkStrength_values[-3] !=0 and blinkStrength_values[-2] !=0:
                ...
                #print ("Predict value: [ "+str(value)+" ]")
                all_values += 1
                if value==0 and L==0:
                    print("Left:ON")
                    db.child("Left").set(1)
                    L=1
                elif value==0 and L==1:
                    print("Left:OFF")
                    db.child("Left").set(0)
                    L=0
                elif value==1 and R==0:
                    print("Right:ON")
                    db.child("Right").set(1)
                    R=1
                elif value==1 and R==1:
                    print("Right:OFF")
                    db.child("Right").set(0)
                    R=0
                    
                elif value==3:
                    print("Normal")
                
                '''if value[0] == 2 and R==0:
                    right_values += 1
                    print ("right blink")
                    db.child("Right").set(1)
                    R=1
                elif value[0] == 1 and L==0:
                    print ("left blink")
                    db.child("Left").set(1)
                    L=1
                elif value[0] == 2 and R==1:
                    print ("right blink")
                    db.child("Right").set(0)
                    R=0
                elif value[0] == 1 and L==1:
                    print ("left blink")
                    db.child("Left").set(0)
                    L=0'''
            else:
